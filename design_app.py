...
def classify_signal(signal_path):
    result_label.config(text=f"Result: {'OCD' if prediction == 1 else 'Normal'}")

# Function to open a file dialog and get the path of the EEG signal
def browse_file():
    signal_path = filedialog.askopenfilename(title="Select Images")
    if signal_path:
        # Display the selected file path
        file_path_label.config(text=f"File: {signal_path}")
        classify_signal(signal_path)



# Create the main application window
root = tk.Tk()
root.title("EEG Signal Classifier")

# Browse Images
browse_button = tk.Button(root, text="Browse Images", command=browse_file)
